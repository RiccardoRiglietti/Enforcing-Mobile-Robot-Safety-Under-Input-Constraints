%% Initialization
% In this code we have implemented the ACC problem using the optimal RCBF 
% (Reciprocal Control Barrier Function) considering a trapezoidal profile for
...
for i = 1: max(size(tspan))-1
    ...
    % choose the optimal RCBF based on current values
    [B, dB, h] = opt_RCBF(af, al, vfn, vln, g0, B1, dB1, B2, dB2, B3, dB3, h1, h2, h3);
    
    % numerical values for quadprog
    Vn = double(subs(V, x, x_cbf0));
    LfVn = double(subs(LfV, x, x_cbf0));
    LgVn = double(subs(LgV, x, x_cbf0));
    Bn = double(subs(B, x, x_cbf0));
    LfBn = double(subs(dB(1), [x; aL], [x_cbf0; a_l(i)]));
    LgBn = double(subs(dB(2), x, x_cbf0));
    hn = double(subs(h, x, x_cbf0));
    Frn = double(subs(Fr, x, x_cbf0));
    
    if ~isreal(Bn) || ~isreal(LgBn) || ~isreal(LfBn)
        disp(num2str(i))
    end
    ...
    H = 2*[1/m^2 0; 0 psc];
    F = -2*[Frn/m^2; 0];
    % wrt ZCBF LgBn and LfBn have changed sign
    A = [LgVn -1; LgBn 0; 1 0; -1 0];
    b = [-10*Vn-LfVn; -LfBn+1/Bn; umax*m*g0; umax*m*g0];
